...
router.get('/', function(req, res, next) {
    res.render('menswear', { loggedIn: req.session.user ? true : false });
});
...
router.post('/login', async function(req, res, next) {
    ...
    if(user!== null){
      ...
      res.redirect("/menswear/?loggedIn=true")
    }else{
      res.redirect("/menswear/?msg=fail")
    }
  });
  ...
  router.get('/logout', function(req,res, next){
    if(req.session.user){
      req.session.destroy()
      res.redirect("/menswear/?msg=logout")
    }else {
      res.redirect("/menswear")
    }
  ...
  router.post('/signup', async function(req, res, next) {
    try {
      ...
      res.redirect('/menswear/?signup=true');
    } catch (error) {
      console.log(error)
      res.redirect('/menswear/?msg=fail');
    }
